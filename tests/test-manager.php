...
class TestManager extends WP_UnitTestCase
{
	...
	public function test_asset_filtering()
	{
		...
		foreach ( Manager::get_assets_filtered( $assets, 'head' ) as $asset ) {
			$this->assertMatchesRegularExpression( "/(scripts|stylesheets)-([a-z\-.]*)-head-(external|internal)/", $asset->get_absolute_filepath(), "Return script or stylesheet assets with 'head', 'external' or 'internal' flags." );
		}

		foreach ( Manager::get_assets_filtered( $assets, 'footer' ) as $asset ) {
			$this->assertMatchesRegularExpression( "/(scripts|stylesheets)-([a-z\-.]*)-footer-(external|internal)/", $asset->get_absolute_filepath(), "Return script or stylesheet assets with 'footer', 'external' or 'internal' flags." );
		}
	}
